 ...
 * root view.
 */
export function phaseExtractProjectionDef(job: ComponentCompilationJob): void {
  // TODO: Why does TemplateDefinitionBuilder force a shared constant?
  const compat = job.compatibility === ir.CompatibilityMode.TemplateDefinitionBuilder;
  ...
  if (selectors.length > 0) {
    ...
    if (selectors.length > 1 || selectors[0] !== '*') {
      let def = selectors.map(s => s === '*' ? s : parseSelectorToR3Selector(s));
      defExpr = job.pool.getConstLiteral(literalOrArrayLiteral(def), compat);
    }

    // Create the ngContentSelectors constant.
    job.contentSelectors = job.pool.getConstLiteral(literalOrArrayLiteral(selectors), compat);
